...
(*
  Nome: Lucas Correa Lemos
  Matrícula: 17/0149412

  Nome: Luis Fernando Lamellas
...
(* begin hide *)
Require Import Arith List Recdef PeanoNat Nat.
...
Proof.
  ...
  - unfold sorted_pair_lst. intros. destruct H.
    unfold snd in H0. assumption.
  ...
  - intros. apply le_all_sorted.
    + unfold le_all. intros. apply merge_in in H0.
      destruct H0 as [H1 | H2].
        * simpl fst in H1. unfold sorted_pair_lst in H. destruct H as [H2 H3].
          simpl fst in H2. apply sorted_le_all in H2. unfold le_all in H2.
          simpl snd in H3. apply sorted_le_all in H3. unfold le_all in H3.
          simpl In in H1. destruct H1. subst. apply leb_complete_conv in e0.
            -- lia.
            -- apply leb_complete_conv in e0. apply H2 in H. lia.
        * simpl In in H2. unfold sorted_pair_lst in H. destruct H.
          simpl snd in H0. apply sorted_le_all in H0. unfold le_all in H0.
          apply H0. assumption.
    + apply IHl. unfold sorted_pair_lst. split.
        * simpl fst. unfold sorted_pair_lst in H. simpl fst in H. destruct H.
          assumption.
        * simpl snd. unfold sorted_pair_lst in H. destruct H. simpl snd in H0.
          simpl fst in H. apply leb_complete_conv in e0. apply tail_sorted in H0.
          assumption.         
Qed.
(* end hide *)
...
Proof.
 ...
 - apply merge_sorts. unfold sorted_pair_lst. split.
   + simpl fst. assumption.
   + simpl snd. assumption.
Qed.
...
Proof.
  intros. functional induction (mergesort l).
    - apply perm_refl.
    - apply perm_refl.
    - unfold perm in IHl1. unfold perm in IHl0. unfold perm. intros.
      rewrite merge_num_oc. unfold fst. unfold snd. rewrite <- IHl1.
      rewrite <- IHl0. rewrite num_oc_append. rewrite firstn_skipn.
      reflexivity.
Qed.
